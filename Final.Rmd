...
Using the `ggplot2` and `reshape2` packages
```{r}
library(ggplot2)
library(reshape2)
...
```
scale_x_discrete(breaks=c("White_Population", "African_American_Population", "Native_American_Population", "Asian_American_Population", "Population_Other_Races"), labels=c("Wh", "Af", "Na", "As", "Ot"))
Using the `plyr` packages midwest's population by race
```{r}
library(plyr)
